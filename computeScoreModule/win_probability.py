...
def win_probability(
    predicted_away_margin, scaled_vegas_line, scaled_std_dev_scoring_margin
):
    prob_lose_regulation = norm.cdf(
        predicted_away_margin + rounding_limit,
        scaled_vegas_line,
        scaled_std_dev_scoring_margin,
    )
    prob_win_regulation = norm.cdf(
        predicted_away_margin - rounding_limit,
        scaled_vegas_line,
        scaled_std_dev_scoring_margin,
    )
...
def play_win_probability(home_team, vegas_line, play):
    ...
    scaled_std_dev_scoring_margin = start_std_dev_scoring_margin / sqrt(
        inverse_game_left
    )
...
def extract_game_left(play):
    ...
    if quarter <= 4:
        min_left = (
            (number_of_quarters - quarter) * minutes_in_quarter
            + minutes
